...
const addNewLogsToHead = async <TLog extends Log>(
	...
	for (const logToAdd of sortedLogs) {
		...
		if (
			logHistory.some(
				logInHistory =>
					logInHistory!.blockHash === logToAdd.blockHash && logInHistory!.logIndex === logToAdd.logIndex,
			)
		)
			continue;
...
const ensureBlockhash = <TBlock extends Block, TLog extends Log>(block: TBlock, logs: Array<TLog>) => {
	...
	logs.forEach(log => {
		if (log.blockHash !== block.hash)
			throw new Error(
				`Received log for block hash ${log.blockHash} when asking for logs of block ${block.hash}.`,
			);
	});
};
